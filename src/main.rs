...
fn gather_header_deps(
    project: &Project,
    src: &Path,
) -> Result<Vec<PathBuf>, AnyError> {
    let mut visited = HashSet::new();
    let mut result = Vec::new();
    gather_header_deps_inner(project, src, &mut visited, &mut result)?;
    Ok(result)
}

fn gather_header_deps_inner(
    project: &Project,
    file: &Path,
    ...
    let canon = match file.canonicalize() {
        Ok(c) => c,
        Err(_) => return Ok(()), // if we can't resolve it, just skip
    };
    ...
    let text = match fs::read_to_string(&canon) {
        Ok(t) => t,
        Err(_) => return Ok(()),
    };
    ...
    for line in text.lines() {
        ...
        if let Some(start) = trimmed.find('"') {
            if let Some(end) = trimmed[start + 1..].find('"') {
                ...
                candidates.push(base_dir.join(header_name));

                // 2) Try include_dirs
                for inc in &project.include_dirs {
                    candidates.push(Path::new(inc).join(header_name));
                }
                ...
                for cand in candidates {
                    if cand.exists() {
                        out.push(cand.clone());
                        // Recurse into header
                        gather_header_deps_inner(project, &cand, visited, out)?;
                        break;
                    }
                }
            }
...
fn hash_object(project: &Project, src: &Path) -> Result<String, AnyError> {
    ...
    // Project config that affects compilation
    ...
    // Header dependencies (recursive)
    let headers = gather_header_deps(project, src)?;
    for header in headers {
        if let Ok(hbytes) = fs::read(&header) {
            hasher.update(hbytes);
        }
    }
